...
class World:
    ...
    def load_graph(self, room_graph):
        ...
        for room_id in room_graph:
            ...
            room_graph_target = room_graph[room_id][1]
            if 'n' in room_graph_target:
                self.rooms[room_id].connect_rooms('n', self.rooms[room_graph_target['n']])
            if 's' in room_graph_target:
                self.rooms[room_id].connect_rooms('s', self.rooms[room_graph_target['s']])
            if 'e' in room_graph_target:
                self.rooms[room_id].connect_rooms('e', self.rooms[room_graph_target['e']])
            if 'w' in room_graph_target:
                self.rooms[room_id].connect_rooms('w', self.rooms[room_graph_target['w']])
